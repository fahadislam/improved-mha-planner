...
void initializePlanner(MHAPlanner*& planner,    //fahad
    ...
    bool bsearch = false;
    // boost::shared_ptr<EnvironmentMHA> m_env;
    planner = new MHAPlanner(&env, 1, true);
...
int runPlanner(MHAPlanner* planner, int allocated_time_secs, 
               vector<int>&solution_stateIDs){
    // int bRet = planner->replan(allocated_time_secs, &solution_stateIDs); //fahad

    MHAReplanParams replan_params(allocated_time_secs);
    ...
    int soln_cost;
    int bRet = planner->replan(&solution_stateIDs, replan_params, &soln_cost);
